...
async def delete_todo(user: user_dependency, db: db_dependency, todo_id: int = Path(gt=0)):
    ...
    if todo is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Todo not found")
    #db.query(Todo).filter(Todo.id == todo_id).delete()  <-- daha güvenli olması icin extra kontrol saglar
    db.delete(todo)
    db.commit()
...
def create_todo_with_gemini(todo_string: str):
    ...
    response = llm.invoke(
        [
            HumanMessage(
                content="I will give you a to-do item in the next message. Based on that, I want you to generate a explanation and recommendation message that encourages me to complete it. My next message will be todo: "),
